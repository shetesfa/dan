...
<head>
    ...
    <style>
        * {
        ...
        .scroll-indicator i { margin: 0 5px; color: #ff6b6b; }
        ...
        .portfolio-preview h4 { color: #1a2a3a; margin-bottom: 5px; }
        .portfolio-preview p { font-size: 0.85rem; color: #718096; }
        ...
        @media (max-width: 768px) {
            ...
            .portfolio-preview { flex-direction: column; text-align: center; }
...
<section class="portfolio-hero">
    <div class="container">
        ...
        <p>Browse our collection of previous work - Click any project to order similar design</p>
    ...
    <div class="container">
        ...
        <div class="gallery-wrapper" id="galleryWrapper">
            <div class="gallery-container" id="galleryContainer">
                ...
                <div class="portfolio-card" onclick="openOrderModal(<?php echo $item['id']; ?>, '<?php echo addslashes($item['title']); ?>', '<?php echo addslashes($item['description']); ?>', '<?php echo addslashes($item['media_url']); ?>', '<?php echo $item['media_type']; ?>', <?php echo $service_id; ?>, '<?php echo addslashes($service['title']); ?>')">
                    <div class="portfolio-media">
                        <?php if($item['media_type'] == 'image'): ?>
                            <img src="<?php echo $item['media_url']; ?>" alt="<?php echo htmlspecialchars($item['title']); ?>">
                        <?php else: ?>
                            <video src="<?php echo $item['media_url']; ?>" poster="<?php echo $item['media_url']; ?>?v=thumb"></video>
                    ...
                    <div class="portfolio-info">
                        ...
                        <button class="btn-order-portfolio" onclick="event.stopPropagation(); openOrderModal(<?php echo $item['id']; ?>, '<?php echo addslashes($item['title']); ?>', '<?php echo addslashes($item['description']); ?>', '<?php echo addslashes($item['media_url']); ?>', '<?php echo $item['media_type']; ?>', <?php echo $service_id; ?>, '<?php echo addslashes($service['title']); ?>')">
                            Order This Design <i class="fas fa-arrow-right"></i>
                        </button>
                    </div>
                </div>
                <?php endwhile; ?>
        ...
        <div class="scroll-indicator">
            <i class="fas fa-arrow-left"></i> Drag to scroll horizontally <i class="fas fa-arrow-right"></i>
        </div>
        <?php else: ?>
        <div class="no-portfolio" style="text-align: center; padding: 80px; background: white; border-radius: 20px;">
            ...
            <a href="services.php" class="back-link" style="background: #ff6b6b; margin-top: 20px; display: inline-block;">Back to Services</a>
        </div>
        <?php endif; ?>
    </div>
</section>
    ...
    <div class="modal-container">
        ...
        <div class="modal-body">
            ...
            <form id="orderForm" onsubmit="return false;">
                ...
                <input type="hidden" id="portfolio_title" name="portfolio_title">
                <input type="hidden" id="portfolio_media" name="portfolio_media">
...
function openOrderModal(portfolioId, title, description, mediaUrl, mediaType, serviceId, serviceTitle) {
    currentPortfolioId = portfolioId;
    
    const previewMedia = document.getElementById('previewMedia');
    if (mediaType === 'video') {
        previewMedia.outerHTML = `<video id="previewMedia" src="${mediaUrl}" style="width:80px;height:80px;object-fit:cover;border-radius:12px;"></video>`;
    } else {
    ...
    document.getElementById('portfolio_title').value = title;
    document.getElementById('portfolio_media').value = mediaUrl;
...
async function submitOrder() {
    const portfolio_id = document.getElementById('portfolio_id').value;
    const portfolio_title = document.getElementById('portfolio_title').value;
    const portfolio_media = document.getElementById('portfolio_media').value;
    ...
    formData.append('portfolio_title', portfolio_title);
    formData.append('portfolio_media', portfolio_media);
    ...
    try {
        const response = await fetch('submit_portfolio_order.php', {
            method: 'POST',
            headers: {
